...
class IL2CPPAnnotateTask(BackgroundTaskThread):
    ...
    def load_types(self, header: str):
        ...
        for t_name in tys.types:
            existing_type = self.bv.get_type_by_name(t_name)
            new_type = tys.types[t_name]
            is_new_full = (
                new_type.type_class in (TypeClass.StructureTypeClass, TypeClass.EnumerationTypeClass)
                and (
                    new_type.type_class != TypeClass.StructureTypeClass 
                    or len(new_type.members) > 0
                )
            )
            is_existing_fwd = (
                not existing_type
                or existing_type.type_class == TypeClass.NamedTypeReferenceClass
                or (
                    existing_type.type_class == TypeClass.StructureTypeClass
                    and len(existing_type.members) == 0
                )
            )
            if not existing_type or (is_new_full and is_existing_fwd):
                self.bv.define_user_type(t_name, new_type)
        return tys
...
class IL2CPPAnnotateAllTask(BackgroundTaskThread):
    def __init__(self, bv: BinaryView):
        super().__init__("Annotating all functions", True)
        self.bv = bv
    ...
    def run(self):
        if script_data is None or "ScriptMethod" not in script_data:
            log_error("IL2CPPDumper data not loaded.")
            return

        methods = script_data["ScriptMethod"]
        total = len(methods)
        log_info(f"Starting annotation of {total} methods...")
        ...
        try:
            ...
            for i in range(0, total, batch_size):
                if self.cancelled:
                    log_info("Annotate All cancelled.")
                    break
                
                batch = methods[i : i + batch_size]
                self.progress = f"Annotating methods: {i}/{total} ({100 * i / total:.1f}%)"
                
                # Find all referenced types in this batch
                batch_refs = set()
                for method in batch:
                    signature = method["Signature"].replace("* method);", "* __method);")
                    try:
                        refs = find_refs(parser.parse(str.encode(signature)).root_node)
                        for ref in refs:
                            batch_refs.add(ref.decode("utf-8"))
                    except Exception as e:
                        pass
                
                # Build struct definitions for all referenced types in this batch
                header = "#include <stdint.h>\n"
                mark = set()
                for ref_name in batch_refs:
                    header += build_struct(ref_name, mark)
                
                # Construct a single header with all signatures in the batch
                for method in batch:
                    signature = method["Signature"].replace("* method);", "* __method);")
                    header += signature + "\n"
                ...
                try:
                    ...
                    for method in batch:
                        ...
                        try:
                            # Extract the function name in the signature to look up in tys.functions
                            # The function name is the last whitespace-separated token before the open parenthesis
                            name_in_sig = signature.split('(')[0].strip().split()[-1]
                            ty_f = tys.functions.get(name_in_sig)
                            if ty_f is not None:
                                fn = self.bv.get_function_at(addr)
                                if fn is None:
                                    fn = self.bv.add_function(addr)
                            ...
                            log_info(f"Failed to apply signature for {name} at {addr}: {str(e)}")
                except Exception as e:
                    log_info(f"Failed to parse signature batch starting at index {i}: {str(e)}")
        finally:
            self.bv.commit_undo_actions()
            self.bv.update_analysis()
            log_info("Annotating all functions finished.")


def annotate_all(bv: BinaryView):
    task = IL2CPPAnnotateAllTask(bv)
    task.start()


def annotate_all_valid(bv: BinaryView):
    return script_data is not None and len(ilp.typedefs) > 0
...
def register():
    ...
    PluginCommand.register(
        "IL2CPPDumper Annotate All", "Process File", annotate_all, annotate_all_valid
    )
