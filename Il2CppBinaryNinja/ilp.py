...
def _get_cpp_parser():
    ...
    try:
        return tree_sitter_languages.get_parser("cpp")
    except Exception as e:
        ts_languages_dir = os.path.dirname(tree_sitter_languages.__file__)
        dll_names = ["languages.dll", "languages.so", "languages.dylib"]
        dll_path = None
        for name in dll_names:
            p = os.path.join(ts_languages_dir, name)
            if os.path.exists(p):
                dll_path = p
                break
        
        if dll_path is None:
            for f in os.listdir(ts_languages_dir):
                if f.startswith("languages."):
                    dll_path = os.path.join(ts_languages_dir, f)
                    break

        if dll_path is None:
            raise RuntimeError(f"Could not find languages shared library in {ts_languages_dir}") from e

        lib = cdll.LoadLibrary(dll_path)
        func = getattr(lib, "tree_sitter_cpp")
        func.restype = c_void_p
        
        lang_ptr = func()
        cpp_lang = Language(lang_ptr)
...
def build_struct(name: str, mark: set[str]) -> str:
    ...
    ret = ""
    decl = typedefs[name_bytes]
    ret += f"// BEG {name}\n"
    for dep, is_strong in decl.refs.items():
        dep_str = dep.decode("utf-8")
        dep_decl = typedefs.get(dep)
        
        if dep_decl is None:
            if is_strong:
                ret += build_struct(dep_str, mark)
            else:
                if dep_str not in mark:
                    ret += f"struct {dep_str};\n"
                    mark.add(dep_str)
            continue
